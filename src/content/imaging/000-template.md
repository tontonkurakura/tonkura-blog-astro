...
- 疑問への答えを3行で書く。見出しは付けない。ここだけ読んで用が足りる状態にする。
...
**この見本について。** 「この疑問が生じる場面」という節は作らない（蛇足になる）。`出典` と `関連する記事` という H2 も本文で使わない（ページ側が自動で出す）。出典は frontmatter の `sources` に入れ、本文に引用マーカーを置かない。`kind` が `docs` か `paper` のもの（一次資料）が最低1件ないとビルドが落ちる。
