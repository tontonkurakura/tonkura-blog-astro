...
sources:
  ...
  - title: anthropics/claude-code-action
    ...
    kind: repo
related: []
# 全記事は Issue を通す。実記事では該当 Issue の番号を書く。
fromIssue: 5
---

> **これは実際の記事ではない。** `fromIssue` はスキーマを満たすための仮値である。 frontmatter の項目と本文テンプレの描画を確かめるために置いている見本である。一覧からは除外され、`draft: true` なので本番ビルドにも出ない。削除するとコレクションが空になり、スキーマの型崩れに気づけなくなるため残しておくこと。
