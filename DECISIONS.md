...
1. **Phase 0（`/install-github-app`）が未実行。** GitHub App のインストールと
   `CLAUDE_CODE_OAUTH_TOKEN` の登録が済むまで、執筆・検証のワークフローは動かない。
   **必ずこのリポジトリのディレクトリで `claude` を起動して実行すること。**
   2026-08-21 に `~/dev`（git 管理外）で開いたセッションから実行して失敗している。
   このコマンドは cwd のリポジトリを対象にするため、`~/dev` では接続先が無い。
2. **お手本記事が未執筆。** 本人と対話しながら1本書く方針で合意済み（2026-08-21）。
   出典を実際に当たり、本人の声で書く。Claude の代筆にはしない。
   これが Phase 2 のスキルを詰めるときの基準になるので、Phase 2 より先にやる。
3. **AI 生成であることを記事に明記するか。** 「Claude が下書きし、筆者が検証・加筆した」の
   一行を入れるかどうか。public な医学系ブログなので入れる方に寄せたいが、未決。
4. **図をどうするか。** 方法論の解説は図がないと辛い。Mermaid を Astro で
   レンダリングできるようにすれば Claude が図まで書ける。
5. **過去の会話の棚卸し。** 埋もれた疑問をまとめて Issue 化する初期移行をやるか。
